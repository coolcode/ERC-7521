 ...
 */
abstract contract MulticallCore {
     ...
     */
    function _validateMulticall(bytes calldata segmentData) internal pure {
     ...
     */
    function _executeMulticall(address, /*intentSender*/ bytes calldata segmentData) internal {
        if (segmentData.length > 32) {
            unchecked {
                bytes memory callData = getSegmentBytes(segmentData, 32, segmentData.length - 32);
                bool success = Exec.call(address(multicall), 0, callData, gasleft());
 ...
 */
contract MulticallDelegate is MulticallCore, IIntentStandard {
    ...
    function validateIntentSegment(bytes calldata segmentData) external pure override {
        _validateMulticall(segmentData);
    }
 ...
 */
function encodeMulticallData(bytes32 standardId, bytes memory callData) pure returns (bytes memory) {
